...
func TripUpdatePbSlice(file string) []realtime.TripUpdate {
...
func TripUpdatePbMap(file string) map[string]realtime.TripUpdate {
	// gtfs-realtime ファイルをバイナリ形式で読み込む
	data, err := os.ReadFile(file)
	if err != nil {
		fmt.Println("ファイルの読み込みエラー:", err)
	...
	// mapの初期化
	res := make(map[string]realtime.TripUpdate)
	...
	for _, entity := range feed.Entity {
		...
		res[*trip.TripId] = realtime.TripUpdate{
			Trip:           trip,
			Vehicle:        vehicle,
			StopTimeUpdate: stopTimeUpdate,
			TimeStamp:      timeStamp,
