...
type TransferGeom struct {
	...
	TransferType    int    `gorm:"not null"`
	MinTransferTime *int
	FromStop        StopGeom `gorm:"foreignKey:FromStopId;references:StopId"`
	ToStop          StopGeom `gorm:"foreignKey:ToStopId;references:StopId"`
}
...
func ParseTransfersGeom(path string) ([]TransferGeom, error) {
	// CSV を開く
	df, err := csvutil.OpenCSV(path)
	if err != nil {
		return nil, fmt.Errorf("failed to open transfers CSV: %w", err)
	...
	for i := 0; i < len(df.Records); i++ {
		...
		// Transfer 構造体を作成しリストに追加
		transfers = append(transfers, TransferGeom{
			FromStopId:      fromStopId,
			ToStopId:        toStopId,
			TransferType:    transferType,
			MinTransferTime: minTransferTime,
