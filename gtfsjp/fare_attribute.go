...
type FareAttribute struct {
	...
	CurrencyType     string  `gorm:"default:JPY"`
	PaymentMethod    int     `gorm:"not null"`
	Transfers        *int    // Required field, but may be NULL if empty.
	TransferDuration *int
	FareRule         []FareRule `gorm:"foreignKey:FareId;references:FareId"`
}
...
type FareAttributeGeom struct {
	...
	Transfers        *int    // Required field, but may be NULL if empty.
	TransferDuration *int
	FareRule         []FareRuleGeom `gorm:"foreignKey:FareId;references:FareId"`
}

func (FareAttributeGeom) TableName() string {
	return "fare_attributes"
}

func ParseFareAttributesGeom(path string) ([]FareAttributeGeom, error) {
	// CSV を開く
	df, err := csvutil.OpenCSV(path)
	if err != nil {
		return nil, fmt.Errorf("failed to open fare_attributes CSV: %w", err)
	...
	// データを解析して FareAttribute 構造体のスライスを作成
	var fareAttributes []FareAttributeGeom
	for i := 0; i < len(df.Records); i++ {
		fareId, err := df.GetString(i, "fare_id")
		if err != nil {
			return nil, fmt.Errorf("failed to get 'fare_id' at row %d: %w", i, err)
		...
		// FareAttribute 構造体を作成しリストに追加
		fareAttributes = append(fareAttributes, FareAttributeGeom{
			FareId:           fareId,
			Price:            price,
			CurrencyType:     currencyType,
			PaymentMethod:    paymentMethod,
