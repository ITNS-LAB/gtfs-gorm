...
func (FrequencyGeom) TableName() string {
	return "frequencies"
}

func ParseFrequenciesGeom(path string) ([]FrequencyGeom, error) {
	// CSV を開く
	df, err := csvutil.OpenCSV(path)
	if err != nil {
		return nil, fmt.Errorf("failed to open frequencies CSV: %w", err)
	...
	for i := 0; i < len(df.Records); i++ {
		...
		// Frequency 構造体を作成しリストに追加
		frequencies = append(frequencies, FrequencyGeom{
			TripId:      tripId,
			StartTime:   startTime,
			EndTime:     endTime,
			HeadwaySecs: headwaySecs,
