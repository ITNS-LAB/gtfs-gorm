...
type RouteGeom struct {
	...
	ContinuousDropOff *int
	NetworkId         *string         `gorm:"index;unique"`
	Trips             []TripsGeom     `gorm:"foreignKey:RouteId;references:RouteId"`
	FareRules         []FareRulesGeom `gorm:"foreignKey:RouteId;references:RouteId"`
	//FareLeg                       FareLeg          `gorm:"foreignKey:NetworkId;references:NetworkId"`
	//FareLegJoinRulesFromNetworkID FareLegJoinRules `gorm:"foreignKey:FromNetworkId;references:NetworkId"`
	//FareLegJoinRulesToStopID      FareLegJoinRules `gorm:"foreignKey:ToStopId;references:NetworkId"`
	RouteNetwork        []RouteNetworkGeom `gorm:"foreignKey:RouteId;references:RouteId"`
...
func ParseRoutesGeom(path string) ([]RouteGeom, error) {
	...
	for i := 0; i < len(df.Records); i++ {
		...
		// Route構造体を作成しリストに追加
		routes = append(routes, RouteGeom{
			RouteId:           routeId,
			AgencyId:          agencyId,
			RouteShortName:    routeShortName,
			RouteLongName:     routeLongName,
