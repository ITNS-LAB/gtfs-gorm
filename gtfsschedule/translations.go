...
func (TranslationGeom) TableName() string {
	return "translations"
}

func ParseTranslationGeom(path string) ([]TranslationGeom, error) {
	// CSVを開く
	df, err := csvutil.OpenCSV(path)
	if err != nil {
		return nil, fmt.Errorf("failed to open translation CSV: %w", err)
	...
	// データを解析してTranslation構造体のスライスを作成
	var translations []TranslationGeom
	for i := 0; i < len(df.Records); i++ {
		tableName, err := df.GetString(i, "table_name")
		if err != nil {
			return nil, fmt.Errorf("failed to get 'table_name' at row %d: %w", i, err)
		...
		// Translation 構造体を作成しリストに追加
		translations = append(translations, TranslationGeom{
			Tablename:   tableName,
			FieldName:   fieldName,
			Language:    language,
			Translation: translation,
