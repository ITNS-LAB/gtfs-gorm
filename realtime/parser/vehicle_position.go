...
func VehiclePositionPbSlice(file string) []orm.VehiclePosition {
	...
	var res []orm.VehiclePosition

	for _, entity := range feed.Entity {
		var trip orm.VehiclePositionTripDescriptor
		var vehicle orm.VehiclePositionVehicleDescriptor
		var position orm.Position
		...
		res = append(res, orm.VehiclePosition{
			Trip:                trip,
			Vehicle:             vehicle,
			Position:            position,
...
func VehiclePositionPbMap(file string) map[string]orm.VehiclePosition {
	// gtfs-realtime ファイルをバイナリ形式で読み込む
	data, err := os.ReadFile(file)
	if err != nil {
		fmt.Println("ファイルの読み込みエラー:", err)
	...
	// mapの初期化
	res := make(map[string]orm.VehiclePosition)
	...
	for _, entity := range feed.Entity {
		...
		res[*trip.TripId] = orm.VehiclePosition{
			Trip:                trip,
			Vehicle:             vehicle,
			Position:            position,
			CurrentStopSequence: currentStopSequence,
